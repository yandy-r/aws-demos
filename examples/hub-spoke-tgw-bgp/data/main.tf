...
locals {
  ...
  cloud_config = [for v in data.template_file.cloud_config : v.rendered]
}
...
data "template_file" "cloud_config" {
  ...
  vars = {
    ...
    public_ssh_key  = data.local_file.public_ssh_key.content
    private_ssh_key = data.local_file.private_ssh_key.content
  }
}
...
data "local_file" "public_ssh_key" {
  filename = pathexpand("${var.priv_key_path}/${var.key_name}.pub")
}
